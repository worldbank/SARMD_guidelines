...
In this chapter, we present a metadata analysis focusing on the household surveys' ability to measure food, non-food, durables, and housing expenditures through different data collection methods.[^agradece] The characteristics of each household survey and their ability to construct a nominal consumption aggregate were collected by filling a metadata formulary. This metadata collection tool may be accessed in the folder [metadata_collection_tool](https://github.com/worldbank/SARMD_guidelines/tree/master/metadata_collection_tool). 

- We provide a METADATA FORMULARY (BLANK) and an example of a filled out formulary that you may use as reference. See "SAR_PAK_2015 (Example).xlsm".
...
[^agradece]: We would like to thank Gabriel Nicolás Camargo Toledo for his support in the conduction of this metadata analysis. 
