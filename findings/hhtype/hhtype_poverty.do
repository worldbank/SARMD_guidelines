	...
	if ("`country'" == "IND") {
		...
		*cap {*/
			...
			*SINGLE PARENTS - MOTHER
			clonevar fsparch=sparch
			replace fsparch=0 if sparch==1&relationharm<3&male==1
			bys countrycode year idh : egen fsparch_m=min(fsparch)
			replace fsparch=0 if fsparch_m==0
			drop fsparch_m msparch_m
			...
			* grand parents and grand children 
			...
			label define hhtype 1 "one person" 2 "couple only" 3 "couple and children only" 4 "single parents" 5 "three generation" 6 "extended family" 7 "non relevant"
