...
```{r  bicyclef,  echo=FALSE, fig.cap = "Average of harmonized variables at subnational level", out.height='10%', fig.align = 'left'}
knitr::include_graphics("figures/fake_figure.PNG")
```

## Complementary visualizations of this book {-}

Many of the graphs in this book are interactive Tableau dashboards. As the example above (Figure \@ref(fig:bicyclef)), you can see not only share of households witn access to `bicycle` in each subnational region of SAR, but also you may filter the results by a differente asset such as `computer` of `cellphone`, or even select a different category of variable such as `demographic` or `education` variables.By using the filters provided you may explore SARMD and learn what kind of data is available to study household welfare.
