...
Household surveys capture a wide variety of development indicators in such areas as poverty, food security, labor market performance, demography, education, health, gender equality, housing and agriculture. In this chapter, we present the results of a Metadata analysis focusing mostly on the household surveys' ability to measure food, non-food, durables, and housing expenditures with different data collection methods. 
...
library(readr)
food <- read_csv("~/SARMD_User_Guidelines/SARMD_guidelines/tables/food.csv", col_types = cols())
...
[![](figures/Food.png)](https://tab.worldbank.org/#/site/WBG/views/SAR_MNA_Metadata/Food "Metadata Analysis: Food Click on this figure to interact with the Tableau dashboard attached.")
...
The second component, non-food expenditures, includes frequently purchased goods and services like soap, cleaning supplies, newspapers, and personal care items. It also includes some less frequent but regular purchases items like clothing, footwear, kitchen equipment, curtains, bedcovers, etc.  In most cases, the non-food expenditures component must be constructed by aggregating expenditures on goods and services from different sections of a survey. 
...
library(readr)
nonfood <- read_csv("~/SARMD_User_Guidelines/SARMD_guidelines/tables/nonfood.csv", col_types = cols())
...
[![](figures/Nonfood.png)](https://tab.worldbank.org/#/site/WBG/views/SAR_MNA_Metadata/NonFood "Click")
...
library(readr)
durables <- read_csv("~/SARMD_User_Guidelines/SARMD_guidelines/tables/durables.csv", col_types = cols())
...
[![](figures/Durables.png)](https://tab.worldbank.org/#/site/WBG/views/SAR_MNA_Metadata/Durables "Click")
...
library(readr)
housing <- read_csv("~/SARMD_User_Guidelines/SARMD_guidelines/tables/housing.csv", col_types = cols())
knitr::kable(housing,booktabs=TRUE, caption = 'Housing')
```

The most relevant dwelling characteristics displayed in the [Tableau dashboard](https://tab.worldbank.org/#/site/WBG/views/SAR_MNA_Metadata/Housing) are:
...
[![](figures/Housing.png)](https://tab.worldbank.org/#/site/WBG/views/SAR_MNA_Metadata/Housing "Click")
